...
contract DutchReverseAuction is Constants, IAuction, VDutchAuction {
    // In the Reverse Dutch, one must bid a lower value than the current price.
    function betterPriceThanCurrent(uint256 b) internal override returns (bool){
        ...
        uint256 increment = ((now - s_auction_start) / s_time_delta) * s_price_delta;

        // The current price is max(s_limit_price, s_starting_price - decrement).
        // Checking underflow
        if (increment > s_starting_price){
            // The increment is higher than the starting price
            return (b <= s_limit_price);
        }

        uint256 current_price = s_starting_price - increment;
